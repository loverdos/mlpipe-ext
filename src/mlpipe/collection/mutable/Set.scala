...
object Set {
  ...
  @inline final def ofIterable[A]: Iterable[A] ⇒ ScalaSet[A] = _.to[ScalaSet]

  @inline final def ofIterator[A]: Iterator[A] ⇒ ScalaSet[A] = _.to[ScalaSet]

  @inline final def ofSeq[A]: Seq[A] ⇒ ScalaSet[A] = _.to[ScalaSet]

  @inline final def ofList[A]: List[A] ⇒ ScalaSet[A] = _.to[ScalaSet]

  @inline final def ofArray[A]: Array[A] ⇒ ScalaSet[A] = _.to[ScalaSet]

  @inline final def ofMap[A, B]: Map[A, B] ⇒ ScalaSet[(A, B)] = _.to[ScalaSet]
