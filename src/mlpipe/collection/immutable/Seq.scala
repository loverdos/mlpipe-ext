...
object Seq {
  ...
  @inline final def unique[A]: ScalaSeq[A] ⇒ ScalaSeq[A] =
    seq ⇒ {
  ...
  @inline final def sortBy[A, B](f: (A) ⇒ B)(implicit ord: Ordering[B]): ScalaSeq[A] ⇒ ScalaSeq[A] = _.sortBy(f)(ord)
  ...
  @inline final def ofMapFilteredValuesByKey[A, B](p: (A) ⇒ Boolean): Map[A, B] ⇒ ScalaSeq[B] =
    it ⇒ ScalaSeq((for((k, v) ← it if p(k)) yield v).toSeq:_*)

  @inline final def ofMapValues[A, B]: Map[A, B] ⇒ ScalaSeq[B] = it ⇒ ScalaSeq(it.values.toSeq:_*)
